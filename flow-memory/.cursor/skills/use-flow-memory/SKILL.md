...
- **公共契约**：读库闸门、写库触发、目录语义、共用禁止项以规则 **`flow-memory`** 为准（全文以此为准，下文不重复闸门条目）。
- **按名字引用**：提及其它规则或技能时仅用名称（如 **`flow-memory`**、**`archive-flow-memory`**），不使用仓库内文件路径。

## 启用条件

当用户消息满足规则 **`flow-memory`** 中的 **读库闸门（Read Gate）** 且意图为**只读参考**（非归档）时启用本技能。

**弱触发（不要启用）**：仅「帮我写 API」「重构这段代码」等，未点名记忆且未指向 flow-memory。

## 不适用

- 未满足规则 **`flow-memory`** 的读库闸门。
...
以下话术有助于同时满足规则 **`flow-memory`** 的读库闸门：
