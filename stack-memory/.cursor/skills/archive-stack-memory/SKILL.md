...
- **公共契约**：分层语义、权力顺序、共用禁止项、**Root（候选路径列表与解析算法）**以规则 **`stack-memory`** 为准；**何时写入**由本技能定义（规则不再重复触发条件）。
- **按名字引用**：仅用名称指涉规则或其它技能（如 **`stack-memory`**、**`use-stack-memory`**），不使用仓库内文件路径。
...
遵守规则 **`stack-memory`** 中的 **L0 > L1 > L2 > L3**。写入内容不得与 L0/L1 冲突；若冲突须在 L3 标注「待与 L1 对齐」而非覆盖硬规则。
...
- 不调用契约 **`stack-memory`** 未声明、未与用户约定的其它规则或技能。

## Root 解析（必须先执行）

- **候选列表、解析顺序、合法性检查**与技能 **`use-stack-memory`** 相同：均以规则 **`stack-memory`** 中「Root（可变锚点）」为准。
- **第一步**：按规则列表自上而下探测；第一个同时包含根下 **`INDEX.md`** 与目录 **`L1_project_knowledge/`** 的路径即为 **root**。
- **禁止**在列表外猜测路径；**禁止**自动创建 root；**禁止**在解析失败时创建任何文件。
...
2. 仅向用户输出**错误说明**（与 **`use-stack-memory`** 中「Root 解析失败」结构一致）：诊断、修复选项、默认可复制候选列表。
